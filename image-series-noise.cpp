...
size_t stochastic_round(double const val) {
    ...
    return std::max(0.0, std::round(val + rng.variate<double, std::uniform_real_distribution>(-.5,.5)));
...
cv::Mat_<float> make_noise(double const mean) {
    cv::Mat_<float> img(3000, 4096, float(0));
    for (int row = 0; row < img.rows; ++row) {
        for (int col = 0; col < img.cols; ++col) {
            img(row, col) = (mean*float(col))/4096.0;
        }
    }
    addNoise(img);
    return img;
}
void synthetic_consistency(std::string prefix) {
    cv::Mat_<float> img1 = make_noise(16);
    ...
    for (double mean = 32; mean < 4097; mean *= 1.25) {
        for (size_t kk = 0; kk < 50; ++kk) {
            runningstats::RunningStats mean_stats, diff_stats;
            cv::Mat_<float> const _img2 = make_noise(mean);
            ...
            if (mean1 / mean2 < 1.2 && mean2 / mean1 < 1.2) {
                ...
                std::cout << "Input mean: " << mean << std::endl
                          << "kk: " << kk << std::endl;
                std::cout << "mean: " << mean_stats.print() << std::endl
    ...
    /*
    diff_by_mean.plotHist(std::string(prefix) + "-diff-by-mean", diff_by_mean.FreedmanDiaconisBinSize(),
            runningstats::HistConfig().setLogCB().setXLabel("diff").setYLabel("mean").setMinMaxX(-3000, 3000));
    */
    /*
    rel_noise_stats.plotHistAndCDF(std::string(prefix) + "-rel-noise", rel_noise_stats.FreedmanDiaconisBinSize(),
            runningstats::HistConfig().setDataLabel("Relative Noise [%]"));

    mean_diff_stats.plotHistAndCDF(std::string(prefix) + "-mean-diff", mean_diff_stats.FreedmanDiaconisBinSize(),
            runningstats::HistConfig().setDataLabel("Difference of Mean Values [%]"));
    */
    {
        std::ofstream out(std::string(prefix) + "-rel-noise-stats.txt");
        out << "# 1. mean 2. stddev" << std::endl
            << rel_noise_stats.getMean() << "\t" << rel_noise_stats.getStddev() << std::endl;
    }
    {
        std::ofstream out(std::string(prefix) + "-mean-diff-stats.txt");
        out << "# 1. mean 2. stddev" << std::endl
            << mean_diff_stats.getMean() << "\t" << mean_diff_stats.getStddev() << std::endl;
    }
    {
        std::ofstream out(std::string(prefix) + "-stats-by-value.txt");
    ...
    {
        std::ofstream out(std::string(prefix) + "-all-stats.txt");
...
int main(int argc, char ** argv) {
    ...
    */
    ...
    if (std::string("--synth") == std::string(argv[2])) {
        synthetic_consistency(argv[1]);
        return EXIT_SUCCESS;
    }
